...
class InteractiveCube(plt.Axes):
        ...
        for move in moves:
            ...
            if np.any(self._digit_flags[:N]):
                for d in np.arange(N)[self._digit_flags[:N]]:
                    self.rotate_face(move.upper(), direction, layer=d)
            else:
                self.rotate_face(move.upper(), direction)
    ...
    def _draw_cube(self):
        ...
        colors = np.asarray(self.cube.face_colors)



        # colors = self.cube.face_colors
...
def _take_input(self, input):
        """Handler for key press events"""
        if input == 'shift':
            self._shift = True
        elif int(input) >= 0 and int(input) <= 9:
            self._digit_flags[int(event.key)] = 1
        elif input == 'R':
            if self._shift:
                ax_LR = self._ax_LR_alt
...
def up_face_modify(up_face):
    up_face_2 = np.empty(9, dtype='U10')

    for i in range(3):
        up_face_2[i*3] = up_face[i]
        up_face_2[i*3 + 1] = up_face[i + 3]
        up_face_2[i*3 + 2] = up_face[i + 2*3]

    temp_str = ""
    
    for char in up_face_2:
        temp_str += char;

    return temp_str


def down_face_modify(down_face):
    down_face_2 = np.empty(9, dtype='U10')
    down_face_3 = np.empty(9, dtype='U10')

    for i in range(3):
        down_face_2[3-i-1] = down_face[3*i]
        down_face_2[3-i-1 + 3] = down_face[3*i + 1]
        down_face_2[3-i-1 + 2*3] = down_face[3*i + 2]


    for i in range(9):
        down_face_3[8-i] = down_face_2[i]

    temp_str = ""
    
    for char in down_face_3:
        temp_str += char;
...
def left_face_modify(left_face):
    left_face_2 = np.empty(9, dtype='U10')

    for i in range(3):
        left_face_2[i*3 + 2] = left_face[i*3];
        left_face_2[i*3] = left_face[i*3 + 2];
        left_face_2[i*3 + 1] = left_face[i*3 + 1];


    temp_str = ""
    
    for char in left_face_2:
        temp_str += char;
...
def right_face_modify(right_face):
    right_face_2 = np.empty(9, dtype='U10')


    for i in range(9):
        right_face_2[8-i] = right_face[i]


    temp_str = ""
    
    for char in right_face_2:
        temp_str += char;
...
def back_face_modify(back_face):
    back_face_2 = np.empty(9, dtype='U10')

    back_face_2[0] = back_face[5]
    back_face_2[1] = back_face[2]
    back_face_2[2] = back_face[4]
    back_face_2[3] = back_face[1]
    back_face_2[4] = back_face[3]
    back_face_2[5] = back_face[0]
    back_face_2[6] = back_face[8]
    back_face_2[7] = back_face[7]
    back_face_2[8] = back_face[6]



    temp_str = ""
    
    for char in back_face_2:
        temp_str += char;
...
def front_face_modify(front_face):
    front_face_2 = np.empty(9, dtype='U10')


    for i in range(9):
        front_face_2[8-i] = front_face[i]


    temp_str = ""
    
    for char in front_face_2:
        temp_str += char;
...
def modification(initial_faces):
    up_face = initial_faces[0]
    down_face = initial_faces[1]
    left_face = initial_faces[2]
    right_face = initial_faces[3]
    back_face = initial_faces[4]
    front_face = initial_faces[5]
    
    up_face_str = up_face_modify(up_face)
    initial_faces[0] = up_face_str

    down_face_str = down_face_modify(down_face)
    initial_faces[1] = down_face_str

    left_face_str = left_face_modify(left_face)
    initial_faces[2] = left_face_str
    
    right_face_str = right_face_modify(right_face)
    initial_faces[3] = right_face_str
    
    back_face_str = back_face_modify(back_face)
    initial_faces[4] = back_face_str

    front_face_str = front_face_modify(front_face)
    initial_faces[5] = front_face_str
...
if __name__ == '__main__':
    ...
    colors = np.empty(54, dtype='U10')

    initial_faces = input("Enter initial face: ")

    initial_faces = initial_faces.split("-");


    # print("\nthis is initial face : \n")
    # print(initial_faces)

    initial_faces = modification(initial_faces)

    # print("\nthis is initial face after modification : \n")
    # print(initial_faces)



    for i in range(len(initial_faces)):
        
        if len(str(initial_faces[i])) != 9:
            print("\nwrong size given at " + str(i) + "th face\n")
            break

        print("\n")
        for j in range(9):
            print(initial_faces[i][j].upper())
            if initial_faces[i][j].upper() == 'W':
                colors[i*9 + j] = 'w'
            elif initial_faces[i][j].upper() == 'Y':
                colors[i*9 + j] = 'yellow'

            elif initial_faces[i][j].upper() == 'B':
                colors[i*9 + j] = 'blue'

            elif initial_faces[i][j].upper() == 'G':
                colors[i*9 + j] = 'green'

            elif initial_faces[i][j].upper() == 'O':
                colors[i*9 + j] = 'orange'

            elif initial_faces[i][j].upper() == 'R':
                colors[i*9 + j] = "red"

    

    c = Cube(N, face_colors = colors)
